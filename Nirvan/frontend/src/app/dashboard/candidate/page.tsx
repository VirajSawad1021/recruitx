      ...
      <div className="grid grid-cols-3 gap-5">
              ...
              <div key={s.name}>
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    {s.verified && <svg className="h-3.5 w-3.5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                  </div>
                  <span className="text-xs text-muted">{s.level}%</span>
                </div>
                <div className="h-2 w-full rounded-full bg-gray-100 overflow-hidden">
                  <div className={`h-full rounded-full transition-all ${s.verified ? "bg-accent" : "bg-accent/40"}`} style={{ width: `${s.level}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-xl border border-card-border bg-white shadow-sm">
          ...
          <div className="p-6 space-y-5">
            <div>
              <p className="text-xs text-muted mb-1">Fit Score Distribution</p>
              <div className="flex items-end gap-1 h-20">
                  {negotiations.map((n, i) => {
                    const recruiter = Array.isArray(n.recruiter) ? n.recruiter[0] : n.recruiter;
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end">
                        <div className="w-full rounded-t-sm bg-gradient-to-t from-accent/70 to-accent transition-all" style={{ height: `${(n.fit_score || 0) * 0.7}px` }} />
                        <span className="text-[9px] text-muted truncate max-w-full">{recruiter?.company?.slice(0, 4) || "?"}</span>
                      </div>
                    );
                  })}
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-card-border/60">
              <div className="rounded-lg bg-subtle p-3 text-center">
                <p className="text-lg font-semibold text-foreground">{negotiations.length}</p>
                <p className="text-[10px] text-muted">Total matches</p>
              </div>
              <div className="rounded-lg bg-subtle p-3 text-center">
                <p className="text-lg font-semibold text-foreground">{negotiations.filter((n) => n.status !== "rejected").length}</p>
                <p className="text-[10px] text-muted">Active / Done</p>
              </div>
            </div>
          </div>
        </div>
