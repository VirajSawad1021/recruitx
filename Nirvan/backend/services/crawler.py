...
async def enrich_profile(candidate_id: str):
    ...
    # If no URLs are provided, we don't need to crawl anything
    if not github_url and not portfolio_url:
        print(f"[Crawler] Candidate {candidate_id} has no GitHub or Portfolio URLs. Skipping.")
        return
    ...
    # 1. Crawl GitHub UI via Jina Reader
    if github_url:
        print(f"[Crawler] Crawling GitHub: {github_url}")
    ...
    if not github_text and not portfolio_text:
        print(f"[Crawler] Both scrapes returned empty. Skipping LLM enrichment.")
        return
